    ...
    renderQuery = () => {
        ...
        if(!form.title){
            ...
            if(code.length == text.length){
                ...
                this.props.actions.contentData(this.props.currentCurId, this.props.currentSection, form.group, { checkboxs: checkboxs, text: form.title });
                message.info('复选框配置成功');
            } else {
    ...
    componentWillMount() {
        ...
        This.props.content.map((d, i) => {
            if (d.layoutId === This.props.currentCurId) {
                let checkboxs = d.children[This.props.currentSection].condition.data.checkboxs;
                let text = '';
                let code = '';
                if(Object.prototype.toString.call(checkboxs) == "[object Array]" && checkboxs.length > 0){
