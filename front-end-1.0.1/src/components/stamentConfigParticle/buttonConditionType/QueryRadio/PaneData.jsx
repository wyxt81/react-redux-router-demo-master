    ...
    renderQuery = () => {
        ...
        if(!form.title){
            ...
            if(code.length == text.length){
                ...
                this.props.actions.contentData(this.props.currentCurId, this.props.currentSection, form.group, { radios: radios, text: form.title });
                message.info('单选按钮配置成功');
            } else {
    ...
    componentWillMount() {
        ...
        This.props.content.map((d, i) => {
            if (d.layoutId === This.props.currentCurId) {
                let radios = d.children[This.props.currentSection].condition.data.radios;
                let text = '';
                let code = '';
                if(Object.prototype.toString.call(radios) == "[object Array]" && radios.length > 0){
